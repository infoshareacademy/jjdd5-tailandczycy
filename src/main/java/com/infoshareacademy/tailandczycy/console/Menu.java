package com.infoshareacademy.tailandczycy.console;

public class Menu {

    public String showMenu(){
        String menuString;


        menuString =""
                        + "============================================================\n"
                        + "||     Welcome to your personal financial analyzer        ||          {\n"
                        + "============================================================       {   }\n"
                        + "||   Choose an operation:                                 ||        }_ { __{\n"
                        + "||        1. Add an Expense                               ||     .'{    }   }'-.\n"
                        + "||        2. Modify Your Expense                          ||    (   }       {   )\n"
                        + "||        3. Delete Your Expense                          ||    |`-.._______..-'|\n"
                        + "||        4. Add a Category for Your com.infoshareacademy.tailandczycy.service.Expense             ||    |               ;--.\n"
                        + "||        5. Delete a Category                            ||    |              (__  \\\n"
                        + "||        6. Display com.infoshareacademy.tailandczycy.service.Expense per Category                ||    |               | )  )\n"
                        + "||        7. Display all com.infoshareacademy.tailandczycy.service.Expense in a given period       ||    |               |/  /\n"
                        + "||        8. Define com.infoshareacademy.tailandczycy.service.Budget                                ||    |               /  /\n"
                        + "||        9. Set up upper limit restriction               ||    |              (  /\n"
                        + "============================================================    \\               y'\n"
                        + "                                                                 `-.._______..-'\n";
        return menuString;
    }
//    public void chooseMenu(int option){
//
//        switch(option){
//            case 1:
//                addExpense();
//                break;
//            case 2:
//                modifyExpense();
//                break;
//            case 3:
//                deleteExpense();
//                break;
//            case 4:
//                addCategory();
//                break;
//              case 5:
//                  deleteCateogory();
//                  break;
//            case 6:
//                displayExpensePerCategory();
//                    break;
//            case 7:
//                displayAllExpenses();
//                break;
//            case 8:
//                defineBudget();
//                  break;
//            case 9:
//                setUpLimit();
//                    break;
//        }
//    }


}
